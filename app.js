...
function normalizeTimestamp(ts) {
  ...
  if (value > 1500000000 && value < nowSec + 86400) return value;
  const asUnixFromMicroPython = value + MICROPYTHON_EPOCH_OFFSET;
  if (asUnixFromMicroPython > 1500000000 && asUnixFromMicroPython < nowSec + 86400) return asUnixFromMicroPython;
...
function computeOnline(updatedAt) {
  ...
  if (delta < -FUTURE_TOLERANCE_SEC) return { online: false, ageSec: Infinity, validTime: false };
  const ageSec = Math.max(0, delta);
  return { online: ageSec <= ONLINE_LIMIT_SEC, ageSec, validTime: true };
}
...
function fmt(ts) {
  ...
  return normalized ? new Date(normalized * 1000).toLocaleString("bg-BG", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }) : "—";
}
...
function renderDiagnostics(current) {
  ...
  if (!node) return;
  const raw = current?.co2_raw ?? "—";
  const corrected = current?.co2_corrected ?? current?.co2 ?? "—";
  const offset = current?.co2_base_offset ?? current?.co2_offset ?? 0;
  const mode = current?.mode || "—";
  const filter = current?.filter || "—";
  const driver = current?.driver || "—";
  const sensorStatus = current?.status !== undefined ? `0x${Number(current.status).toString(16).toUpperCase().padStart(4, "0")}` : "—";
  const confidence = current?.confidence || (Number(offset) ? "calibrated_offset" : "raw_unadjusted");
  node.textContent = `Raw: ${raw} ppm · Corrected: ${corrected} ppm · Offset: ${offset} ppm · Confidence: ${confidence} · Sensor status: ${sensorStatus} · Mode: ${mode} · Filter: ${filter} · Driver: ${driver}`;
}

function render(current, raw) {
  const co2 = Number(current?.co2 || current?.co2_corrected || 0);
  ...
  $("co2").innerHTML = co2 ? `${Math.round(co2)} <span class="unit">ppm</span>` : "—";
  ...
  $("updated").textContent = updatedAt ? `Последно обновяване: ${fmt(updatedAt)} · ${ageText(status.ageSec, status.validTime)} · ${device}` : "Последно обновяване: няма данни";
  renderDiagnostics(current);

  $("dot").classList.toggle("on", online);
  $("conn").textContent = !hasData ? "Няма валидни данни" : online ? "Устройството е онлайн" : "Устройството е офлайн";

  const nowSec = Math.floor(Date.now() / 1000);
  const minTime = nowSec - 12 * 60 * 60;
  const data = Object.values(raw || {})
    .map((item) => ({ ...item, timestamp: normalizeTimestamp(item?.timestamp) }))
    .filter((item) => item && item.timestamp && item.timestamp >= minTime && item.timestamp <= nowSec + 60)
    .sort((a, b) => a.timestamp - b.timestamp);

  const labels = data.map((item) => new Date(item.timestamp * 1000).toLocaleTimeString("bg-BG", { hour: "2-digit", minute: "2-digit" }));
  ...
  chart = new Chart($("chart"), {
    ...
    data: {
      labels,
      datasets: [
        { label: "CO₂ corrected ppm", data: data.map((item) => item.co2), borderColor: "#39d98a", backgroundColor: "rgba(57,217,138,0.14)", fill: true, tension: 0.3, pointRadius: 2, borderWidth: 3, yAxisID: "y" },
        { label: "CO₂ raw ppm", data: data.map((item) => item.co2_raw ?? null), borderColor: "#9db1cb", backgroundColor: "transparent", fill: false, tension: 0.2, pointRadius: 1, borderWidth: 1, yAxisID: "y" },
...
async function loadWeather() {
  try {
    ...
    setText("weatherUpdated", `Метеорологичните данни не са налични: ${error.message}`);
  }
}
